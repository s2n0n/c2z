...
def main():
    ...
    if not services:
        print("[ERROR] No services found in c2z-system or simulation namespaces. Is the cluster running?")
        return
    ...
    for svc in services:
        ...
        if forward_all:
            ...
            # Interactive mode
            user_input = input(f"\n❓ Forward [{namespace}] {svc_name} (Port: {svc_port})? (y/n): ").lower()
            if user_input == 'y':
                should_forward = True
                
                # Check if default port is free
                default_is_free = (svc_port not in used_ports) and (not is_port_in_use(svc_port))
                prompt_default = svc_port if default_is_free else get_available_port(svc_port, used_ports)
        ...
        if should_forward:
            ...
            if local_port in used_ports:
                print(f"   ⚠️  Warning: Port {local_port} was already assigned in this session.")
            ...
            try:
                proc = subprocess.Popen(cmd.split(), stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
                processes.append({
                    "proc": proc,
                    "name": svc_name,
                    "namespace": namespace,
                ...
                print(f"{svc_name:<30} | {namespace:<15} | http://localhost:{local_port} -> {svc_port}")
