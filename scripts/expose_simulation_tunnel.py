...
def start_kubectl_port_forward(service, local_port):
    ...
    try:
        ...
        if proc.poll() is not None:
             ...
             print(f"[ERROR] kubectl port-forward failed immediately for {service['name']}: {stderr.decode()}")
             return False
             
        # Verify connectivity
        if not check_local_connection(local_port):
            print(f"[ERROR] Port-forward process is running but port {local_port} is not accessible.")
            proc.terminate()
            return False
            
        return True
    except Exception as e:
        print(f"[ERROR] Failed to start kubectl port-forward for {service['name']}: {e}")
        return False
